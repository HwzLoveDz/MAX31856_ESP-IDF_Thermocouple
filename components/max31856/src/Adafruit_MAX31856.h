...
typedef uint32_t (*max_com_fptr_t)(spi_device_handle_t spi, uint8_t *data, uint8_t len);
...
public:
  ...
  // Adafruit_MAX31856(gpio_num_t spi_cs, gpio_num_t spi_clk, gpio_num_t spi_mosi, gpio_num_t spi_miso);
  // Adafruit_MAX31856(gpio_num_t spi_cs, gpio_num_t spi_clk, gpio_num_t spi_mosi, gpio_num_t spi_miso);
  Adafruit_MAX31856(void);

  bool begin(max_com_fptr_t read_cb, max_com_fptr_t write_cb, spi_device_handle_t spi);
...
private:
  ...
  max_com_fptr_t _read_cb = nullptr;
  max_com_fptr_t _write_cb = nullptr;
